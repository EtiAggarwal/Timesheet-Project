...
--Procedure to reset password by user

IF ( OBJECT_ID('SP_RESET_PASS_USER') IS NOT NULL ) 
   DROP PROCEDURE SP_RESET_PASS_USER
GO

CREATE PROCEDURE SP_RESET_PASS_USER
       @EMPLOYEE_ID VARCHAR(255),
...
BEGIN 
	...
	@COUNT NUMERIC
			...
			SELECT @COUNT=COUNT(EMPLOYEE_ID) FROM USER_LOGIN WHERE PASS = @OLD_PASS AND EMPLOYEE_ID = @EMPLOYEE_ID;
			IF  @COUNT <> 1
				RETURN -1;  -- invalid password
			ELSE
			BEGIN
				UPDATE USER_LOGIN SET PASS = @NEW_PASS , LAST_MODIFIED_DATE=CURRENT_TIMESTAMP,MODIFIED_BY= @EMPLOYEE_ID WHERE PASS = @OLD_PASS AND EMPLOYEE_ID = @EMPLOYEE_ID ;
				IF @@ROWCOUNT = 1
					RETURN 1;
				ELSE
					RETURN 0;
...
--Procedure to reset user pass by admin

IF ( OBJECT_ID('SP_RESET_USER_PASS_ADMIN') IS NOT NULL ) 
   DROP PROCEDURE SP_RESET_USER_PASS_ADMIN
GO

CREATE PROCEDURE SP_RESET_USER_PASS_ADMIN
       @EMPLOYEE_ID VARCHAR(255),
       @ADMIN_ID VARCHAR(255),
       @NEW_USER_PASS VARCHAR(255),
       @NEW_USER_GUID UNIQUEIDENTIFIER,
       @ADMIM_PASS VARCHAR(255)
AS
BEGIN 
	DECLARE 
	@COUNT NUMERIC

			SELECT @COUNT=COUNT(EMPLOYEE_ID) FROM USER_LOGIN WHERE PASS = @ADMIM_PASS AND EMPLOYEE_ID = @ADMIN_ID;
			IF  @COUNT <> 1
				RETURN -1;  -- invalid admin password
			ELSE
			BEGIN
				UPDATE USER_LOGIN SET PASS = @NEW_USER_PASS , USER_GUID = @NEW_USER_GUID, LAST_MODIFIED_DATE=CURRENT_TIMESTAMP,MODIFIED_BY= @ADMIN_ID WHERE EMPLOYEE_ID = @EMPLOYEE_ID ;
				IF @@ROWCOUNT = 1
					RETURN 1;
				ELSE
					RETURN 0;
...
IF ( OBJECT_ID('SP_UPDATE_USER_TYPE_ADMIN') IS NOT NULL ) 
   DROP PROCEDURE SP_UPDATE_USER_TYPE_ADMIN
GO

CREATE PROCEDURE SP_UPDATE_USER_TYPE_ADMIN
       @EMPLOYEE_ID VARCHAR(255),
       @ADMIN_ID VARCHAR(255),
       @NEW_IS_ADMIN BIT,
       @ADMIM_PASS VARCHAR(255)
...
BEGIN 
	...
	@COUNT NUMERIC
			...
			ELSE
			BEGIN
				UPDATE USER_LOGIN SET IS_ADMIN = @NEW_IS_ADMIN, LAST_MODIFIED_DATE=CURRENT_TIMESTAMP,MODIFIED_BY= @ADMIN_ID WHERE EMPLOYEE_ID = @EMPLOYEE_ID ;
				IF @@ROWCOUNT = 1
					RETURN 1;
				ELSE
					RETURN 0;
...
update USER_LOGIN set IS_ADMIN = 1 where EMPLOYEE_ID=1235
